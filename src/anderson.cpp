...
int main(int argc, char* argv[])
{
   ...
   if (calc_2pgf) {   
	 ...
	 std::string ind_str = boost::lexical_cast<std::string>(ind.Index1) + boost::lexical_cast<std::string>(ind.Index2) +boost::lexical_cast<std::string>(ind.Index4) +boost::lexical_cast<std::string>(ind.Index3);
	 ...
	 { // dispatch and save two-particle GF data - MPI parallelization in bosonic freqs
	    ...
	    for (pMPI::MPIWorker worker(comm,ROOT);!worker.is_finished();) {
		  ...
		  std::ofstream chi_ferm_stream (("chi_ferm_"+ind_str+"_w1_"+boost::lexical_cast<std::string>(p)+".dat").c_str());
		  ...
		  for (int w2_index = -wf_max; w2_index<wf_max; w2_index++) { // loop over second fermionic frequency 
			...
			chi_ferm_stream << std::scientific << std::setprecision(12) << w1_index << " " << w2_index << " " << w1p_index << "   " << std::real(val) << " " << std::imag(val) << std::endl;
		     }
		  };
		  chi_ferm_stream.close();
